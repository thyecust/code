...
import { QNe, Ave, oHt, vve, kve } from "./chunk-xz3v8y7y.js";
...
var u = async (l, o) => {
  ...
  if (e === "") {
    ...
    let r = t.iterations === 0 ? "not yet evaluated" : `${t.iterations} ${R(t.iterations, "turn")}`,
      n = t.lastReason
        ? `
Last check: ${Tr(t.lastReason.trim())}`
        : "";
    return { type: "text", value: `Goal active: ${t.condition} (${r})${n}` };
  }
  if (Ave(e)) {
    let t = kve(o);
    return { type: "text", value: t === null ? "No goal set" : `Goal cleared: ${t}` };
  }
  if (e.length > QNe)
    return (
      g("goal_set", "too_long"),
      { type: "text", value: `Goal condition is limited to ${QNe} characters (got ${e.length})` }
    );
  let a = vve(e, o);
  if (a !== null) return { type: "text", value: a };
  return { type: "query", value: `Goal set: ${e}`, prompt: oHt(e) };
};
export { u as call };
