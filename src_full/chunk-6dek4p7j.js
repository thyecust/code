...
// Version: 2.1.259
import { _e, eR } from "./chunk-x1rrg5j2.js";
...
import { tit } from "./chunk-eawgw36a.js";
...
import { U, d, j } from "./chunk-w71cs9yg.js";
...
j();
import { mkdir as Z, writeFile as tt } from "fs/promises";
import { dirname as et, extname as ot } from "path";
function rt(t) {
  let s = ot(t) === "" ? `${t}.txt` : t,
    i = eR().workspace === "remote" ? _e() : void 0;
  return ct(s, i);
}
...
function vt(ee) {
  return ee.project.cwd;
}
function P(Xt) {
  let x = y(32),
    { content: S, defaultFilename: nt, onDone: u } = Xt,
    [, st] = d(null),
    [E, Yt] = d(nt),
    [B, qt] = d(nt.length),
    [m, at] = d(!1),
  ...
  if (x[1] !== S || x[2] !== u)
    ((pt = async (mt) => {
      if (mt === "clipboard") {
        let ft = await m_(S);
        if (ft) process.stdout.write(ft);
        (_("export_clipboard"), u({ success: !0, message: "Conversation copied to clipboard" }));
      } else if (mt === "file") (st("file"), at(!0));
    }),
      (x[1] = S),
      (x[2] = u),
      (x[3] = pt));
  ...
  if (x[4] !== S || x[5] !== E || x[6] !== u)
    ((gt = async () => {
      try {
        let Wt = await b(E, S);
        (_("export_file"), u({ success: !0, message: `Conversation exported to: ${Wt}` }));
      } catch (k) {
        let ut = k;
  ...
  let xt;
  if (x[12] !== G)
    ((xt = [dt, { label: "Save to file", value: "file", description: G }]),
      (x[12] = G),
      (x[13] = xt));
  else xt = x[13];
  let I = xt,
  ...
  let J = ht,
    wt;
  if (x[16] !== m) ((wt = { context: "Settings", isActive: m }), (x[16] = m), (x[17] = wt));
  else wt = x[17];
  We("confirm:no", C, wt);
  const N = !m;
  let D;
  if (
    ...
    ((D = !m
      ? e(Re, { options: I, onChange: A, onCancel: C })
      : r(o, {
          flexDirection: "column",
          children: [
            e(n, { children: "Enter filename:" }),
            r(o, {
              flexDirection: "row",
              gap: 1,
              marginTop: 1,
  ...
  let yt;
  if (x[27] !== C || x[28] !== J || x[29] !== N || x[30] !== D)
    ((yt = e(me, {
...
function bt(t) {
  let s = t.getFullYear(),
    ...
    w = String(t.getSeconds()).padStart(2, "0");
  return `${s}-${i}-${a}-${l}${h}${w}`;
}
function St(t) {
  let s = t.find((l) => l.type === "user");
  ...
  if (((a = Tr(a)), a.length > 50)) a = a.substring(0, 49) + "\u2026";
  return a;
}
function Et(t) {
  return t
...
async function fe(t, s, i) {
  let a = await tit(s.session, s.messages, s.options.tools || [], { storageV5: s.storageV5 }),
    l = i.trim();
  if (l) {
    try {
      let c = await b(l, a);
      (_("export_file"), t(`Conversation exported to: ${c}`));
    } catch (c) {
      (f("export_file", "write_failed"),
        t(`Failed to export conversation: ${c instanceof Error ? c.message : "Unknown error"}`));
    }
    return null;
  }
  let h = St(s.messages),
...
export { fe as call, St as extractFirstPrompt, Et as sanitizeFilename };
