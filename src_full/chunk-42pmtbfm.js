...
import { b, ro } from "./chunk-fzpv8ev5.js";
...
import { UWe } from "./chunk-wpg07n0k.js";
...
import { f_, ove, $p, Fi, Q2, H0 } from "./chunk-m1g84jsb.js";
...
import { KB } from "./chunk-31rmjnrq.js";
...
import { aH } from "./chunk-gn1spzts.js";
...
import { isAbsolute as k, relative as S, resolve as I } from "path";
async function N(i, s, n) {
  let d = i ? await _l(I(i), Uu(n)) : void 0;
  function c(t) {
    if (!d) return !0;
    let e = S(d, t);
    return e.split(/[/\\]/, 1)[0] !== ".." && !k(e);
  }
  let [u, g, w] = await Promise.all([KB(n), f_(void 0, n), UWe(n)]),
    a = new Map();
  for (let t of u) if (t.kind === "bg" && t.jobId) a.set(t.jobId, t);
  let f = new Set(w.shorts);
  for (let t of a.keys()) f.add(t);
  let o = [],
    l = new Set();
  for (let t of ove(g, f)) {
    let e = a.get(t.id);
    if (e) l.add(e.pid);
    if (!c(Q2(t.state))) continue;
    let r = y(t.state, e?.status);
    if (!s && !e && r !== "working" && r !== "blocked") continue;
    let m = ro(Bo(e?.name ?? t.state.name ?? t.state.displayIntent ?? t.state.intent));
    o.push({
      ...(e && { pid: e.pid }),
      id: t.id,
      cwd: e?.cwd ?? t.state.cwd,
      kind: "background",
      startedAt: e?.startedAt ?? Date.parse(t.state.createdAt),
      sessionId: e?.sessionId ?? t.state.sessionId,
      ...(m && { name: m }),
      ...(e?.status && { status: p(e.status) }),
      ...(e?.status === "waiting" && e.waitingFor && { waitingFor: e.waitingFor }),
      state: r,
    });
  }
  for (let t of u) {
    if ((t.kind !== "interactive" && t.kind !== "bg") || l.has(t.pid)) continue;
    if (t.kind === "bg" && t.jobId) continue;
    if (t.parkedJobId !== void 0) continue;
    if (!c(t.cwd)) continue;
    let e = t.name && ro(Bo(t.name));
    o.push({
      pid: t.pid,
      cwd: t.cwd,
      kind: t.kind === "bg" ? "background" : "interactive",
      startedAt: t.startedAt,
      ...(t.sessionId && { sessionId: t.sessionId }),
      ...(e && { name: e }),
      ...(t.status && { status: p(t.status) }),
      ...(t.status === "waiting" && t.waitingFor && { waitingFor: t.waitingFor }),
    });
  }
  (o.sort((t, e) => t.startedAt - e.startedAt),
    await aH(
      b(o, null, 2) +
        `
`,
...
function p(i) {
  return i === "idle" ? "idle" : i === "waiting" ? "waiting" : "busy";
}
function y(i, s) {
  if (s === "busy") return "working";
  let n = $p(i.state);
  if (Fi(i) && !(n === "success" && H0(i)))
    return n === "success" ? "done" : n === "failure" ? "failed" : "stopped";
  if (i.tempo === "blocked" || s === "waiting") return "blocked";
  return "working";
}
export { N as printAgentsJson };
