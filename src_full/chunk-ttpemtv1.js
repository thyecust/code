...
import { deflateSync as S, inflateRawSync as F } from "zlib";
var z = {
    30: { r: 0, g: 0, b: 0 },
    31: { r: 205, g: 49, b: 49 },
    32: { r: 13, g: 188, b: 121 },
...
`);
  for (let e of n) {
    ...
    while (o < e.length) {
      if (e[o] === "\x1B" && e[o + 1] === "[") {
        let f = o + 2;
        while (f < e.length && !/[A-Za-z]/.test(e[f])) f++;
        if (e[f] === "m") {
          let u = e
              .slice(o + 2, f)
              .split(";")
      ...
      if (e[o] === "\x1B") {
        if (e[o + 1] === "]") {
          let f = o + 2;
          while (f < e.length && e[f] !== "\x07" && !(e[f] === "\x1B" && e[f + 1] === "\\")) f++;
          o = e[f] === "\x1B" ? f + 2 : f + 1;
        } else o++;
        continue;
      }
      let g = o;
      while (o < e.length && e[o] !== "\x1B") o++;
      let a = e.slice(g, o);
...
var D = A();
function A() {
  let t = new Uint8Array(H);
  for (let r = 2; r < m - 4; r++)
    for (let n = 1; n < P - 1; n++)
      if ((r === 2 || r === m - 5 || n === 1 || n === P - 2) && (n + r) % 2 === 0)
        t[r * P + n] = 255;
  return t;
}
var G = N();
function N() {
  let t = F(Buffer.from(Y, "base64")),
    r = t.readUInt16LE(0),
    n = new Map(),
    e = 2;
  for (let i = 0; i < r; i++) {
    let s = t.readUInt32LE(e);
    ((e += 4), n.set(s, t.subarray(e, e + H)), (e += H));
  }
  return n;
}
function fn(t, r = {}) {
  ...
  let g = Math.max(1, ...o.map(U)),
    ...
    f = (g * P + e * 2) * n,
    u = (a * m + i * 2) * n,
    c = new Uint8Array(f * u * 4);
  if ((J(c, l), s > 0)) Z(c, f, u, s * n);
  let d = e * n,
    p = i * n;
  for (let b = 0; b < a; b++) {
    let y = 0;
    for (let h of o[b])
      for (let T of h.text) {
        ...
        let O = d + y * P * n,
          X = p + b * m * n,
          q = k[M];
        if (q !== void 0) B(c, f, O, X, h.color, l, q, n);
        else {
          let j = G.get(M) ?? D;
          K(c, f, O, X, j, h.color, h.bold, n);
        }
        y += w;
      }
  }
...
function U(t) {
  let r = 0;
  for (let n of t) r += se(n.text);
  return r;
}
function J(t, r) {
  for (let n = 0; n < t.length; n += 4)
    ((t[n] = r.r), (t[n + 1] = r.g), (t[n + 2] = r.b), (t[n + 3] = 255));
}
var k = { 9617: 0.25, 9618: 0.5, 9619: 0.75, 9608: 1 };
function B(t, r, n, e, i, s, l, o) {
  let g = Math.round(i.r * l + s.r * (1 - l)),
    a = Math.round(i.g * l + s.g * (1 - l)),
    f = Math.round(i.b * l + s.b * (1 - l)),
    u = P * o,
    c = m * o;
  for (let d = 0; d < c; d++) {
    let p = ((e + d) * r + n) * 4;
    for (let b = 0; b < u; b++) {
      let y = p + b * 4;
      ((t[y] = g), (t[y + 1] = a), (t[y + 2] = f));
    }
  }
}
function K(t, r, n, e, i, s, l, o) {
  for (let g = 0; g < m; g++)
    for (let a = 0; a < P; a++) {
      let f = i[g * P + a];
      if (f === 0) continue;
      if (l) f = Math.min(255, f * 1.4);
      let u = 255 - f;
      for (let c = 0; c < o; c++) {
        let d = ((e + g * o + c) * r + n + a * o) * 4;
        for (let p = 0; p < o; p++) {
          let b = d + p * 4;
          ((t[b] = (s.r * f + t[b] * u) >> 8),
            (t[b + 1] = (s.g * f + t[b + 1] * u) >> 8),
            (t[b + 2] = (s.b * f + t[b + 2] * u) >> 8));
        }
      }
    }
}
function Z(t, r, n, e) {
  ...
  for (let s = 0; s < e; s++)
    for (let l = 0; l < e; l++) {
      let o = e - l - 0.5,
        g = e - s - 0.5;
      if (o * o + g * g <= i) continue;
      ((t[(s * r + l) * 4 + 3] = 0),
        (t[(s * r + (r - 1 - l)) * 4 + 3] = 0),
        (t[((n - 1 - s) * r + l) * 4 + 3] = 0),
        (t[((n - 1 - s) * r + (r - 1 - l)) * 4 + 3] = 0));
    }
}
var E = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]),
  I = Q();
function Q() {
...
function C(t) {
  let r = 4294967295;
  for (let n = 0; n < t.length; n++) r = I[(r ^ t[n]) & 255] ^ (r >>> 8);
  return (r ^ 4294967295) >>> 0;
}
function v(t, r) {
  let n = Buffer.alloc(4 + r.length);
  (n.write(t, 0, "ascii"), n.set(r, 4));
  let e = Buffer.alloc(12 + r.length);
  return (e.writeUInt32BE(r.length, 0), n.copy(e, 4), e.writeUInt32BE(C(n), 8 + r.length), e);
}
function V(t, r, n) {
  let e = Buffer.alloc(13);
  (e.writeUInt32BE(r, 0),
    e.writeUInt32BE(n, 4),
    (e[8] = 8),
    (e[9] = 6),
    ...
    (e[12] = 0));
  let i = r * 4,
    s = Buffer.alloc(n * (i + 1));
  for (let o = 0; o < n; o++) {
    let g = o * (i + 1);
    ((s[g] = 0), s.set(t.subarray(o * i, (o + 1) * i), g + 1));
  }
  let l = S(s);
  return Buffer.concat([E, v("IHDR", e), v("IDAT", l), v("IEND", new Uint8Array(0))]);
}
export {
  m as GLYPH_H,
  P as GLYPH_W,
