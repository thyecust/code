// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.259
import { y } from "./chunk-szxazvwk.js";
import "./chunk-85mjac4q.js";
import "./chunk-x1rrg5j2.js";
import "./chunk-jdw11prg.js";
import "./chunk-56nvyfje.js";
import "./chunk-1mrhsd7s.js";
import { kp } from "./chunk-kn2qhfka.js";
import "./chunk-55w4bsdv.js";
import "./chunk-9fysw8jv.js";
import "./chunk-m92n5xra.js";
import "./chunk-m7w5m1w6.js";
import "./chunk-058caznt.js";
import "./chunk-97tbrkcc.js";
import { t } from "./chunk-fzpv8ev5.js";
import "./chunk-xmrr4sh8.js";
import { R } from "./chunk-ras23w04.js";
import "./chunk-6rkpsn9e.js";
import "./chunk-ck0tqv1m.js";
import { ZC, Z2 } from "./chunk-0f1244y5.js";
import "./chunk-p97n3qf8.js";
import { yn, w9, E9 } from "./chunk-tdg690ym.js";
import { ye } from "./chunk-435ekv5k.js";
import "./chunk-rnzbwgja.js";
import "./chunk-ptdm1fhw.js";
import { lo } from "./chunk-dz1fj2ka.js";
import "./chunk-5dw4kvcq.js";
import "./chunk-8mbwgjdd.js";
import "./chunk-m0a16ehy.js";
import { mHe, jz, gH, Z4e, Fcn } from "./chunk-m86gk47f.js";
import "./chunk-x722nt0q.js";
import "./chunk-1v541dwj.js";
import "./chunk-bx79h7g8.js";
import "./chunk-9f9fskgc.js";
import "./chunk-spz20jb6.js";
import { Il } from "./chunk-mzmfq60a.js";
import "./chunk-616tsvrd.js";
import "./chunk-ye42pw2j.js";
import "./chunk-vdqz95a3.js";
import "./chunk-n8g979s0.js";
import "./chunk-mxy52vze.js";
import "./chunk-zk8esmth.js";
import "./chunk-k1wkanbv.js";
import "./chunk-7y5wjz4e.js";
import "./chunk-gxyczd8c.js";
import "./chunk-h96shwz8.js";
import "./chunk-gzwhm5vd.js";
import "./chunk-nwzn6gxv.js";
import "./chunk-enjww0fp.js";
import "./chunk-0dkpd8qq.js";
import "./chunk-67nd7etf.js";
import "./chunk-7xxnrgeg.js";
import { ARn } from "./chunk-rahwxqh8.js";
import "./chunk-rjxafr3h.js";
import "./chunk-21dppk21.js";
import "./chunk-edmcaynh.js";
import "./chunk-wxd1scze.js";
import "./chunk-77152aqa.js";
import "./chunk-xfn8hpdj.js";
import "./chunk-33da912m.js";
import "./chunk-x67fwt53.js";
import "./chunk-8trhjkwe.js";
import "./chunk-4z2eqcrq.js";
import "./chunk-b3pxzdmn.js";
import "./chunk-pwdby7t2.js";
import "./chunk-ta3hhm0z.js";
import "./chunk-a0qeq8pm.js";
import "./chunk-9qgz04yg.js";
import "./chunk-vv5g97a8.js";
import "./chunk-b5ax9mbm.js";
import "./chunk-35w62chd.js";
import "./chunk-2yqsfgga.js";
import "./chunk-trntcdrz.js";
import "./chunk-nmde69vm.js";
import "./chunk-7r03n5n9.js";
import "./chunk-9pd12rac.js";
import "./chunk-a7a5sap3.js";
import "./chunk-3r19kwqx.js";
import "./chunk-55t63zqr.js";
import "./chunk-5t2g7ar8.js";
import "./chunk-0xd0k64r.js";
import "./chunk-j2rxdvy0.js";
import "./chunk-a3w4apvy.js";
import { o, n } from "./chunk-h8z84shc.js";
import "./chunk-fkx9wvrt.js";
import "./chunk-2kd50vvr.js";
import "./chunk-b4nvhxkz.js";
import "./chunk-5j20p3d9.js";
import "./chunk-5x3q9dya.js";
import "./chunk-bbpye6w7.js";
import "./chunk-0h1h2vpe.js";
import "./chunk-vvzgt3y1.js";
import "./chunk-eby52mzm.js";
import "./chunk-nt36bgfs.js";
import "./chunk-rzhk9vzz.js";
import "./chunk-zxkkdwp1.js";
import "./chunk-yj7edqzj.js";
import "./chunk-1jntcv5v.js";
import "./chunk-xk2tbyg9.js";
import "./chunk-t387wqyr.js";
import "./chunk-4btmh39m.js";
import "./chunk-5z29rkb5.js";
import "./chunk-tgjjqfz6.js";
import "./chunk-s2qjm80t.js";
import { We } from "./chunk-0y8vm6va.js";
import { pe } from "./chunk-mhx2enzs.js";
import "./chunk-jtegkk17.js";
import "./chunk-mq6t5gsy.js";
import "./chunk-3ye1v0g2.js";
import "./chunk-y9yvkcas.js";
import "./chunk-pg32k5cc.js";
import "./chunk-a0rk6xcw.js";
import "./chunk-qpwbvc04.js";
import "./chunk-rfjqgh1w.js";
import { F } from "./chunk-aejk3fcb.js";
import { aR, jGe } from "./chunk-eawgw36a.js";
import "./chunk-57r9gvjy.js";
import "./chunk-npdpr5xp.js";
import { xn } from "./chunk-er3vegm6.js";
import "./chunk-1hjsy6d2.js";
import "./chunk-b6r8zdy5.js";
import "./chunk-1qpkzqbm.js";
import "./chunk-esn2chqn.js";
import "./chunk-1fhzzx7h.js";
import "./chunk-321z9ran.js";
import "./chunk-zwtg7j89.js";
import "./chunk-qyqph83r.js";
import "./chunk-6q0mr3fq.js";
import "./chunk-1k9bsfa7.js";
import "./chunk-cezzeybd.js";
import "./chunk-dtz7ymrp.js";
import "./chunk-7bh7nxpg.js";
import "./chunk-r8hc3n2z.js";
import "./chunk-8nj3fpx9.js";
import "./chunk-wkxp81p6.js";
import "./chunk-f1ybk250.js";
import "./chunk-zj5hwn6k.js";
import "./chunk-svmkhj92.js";
import "./chunk-dybqhddz.js";
import "./chunk-31rmjnrq.js";
import "./chunk-q0fs2qb2.js";
import "./chunk-a33zz456.js";
import "./chunk-5rcp9861.js";
import "./chunk-xz3v8y7y.js";
import "./chunk-jqjjqb8p.js";
import "./chunk-6dcxrmbt.js";
import "./chunk-jkrywj2s.js";
import "./chunk-hnqd864x.js";
import "./chunk-hrhk4q9h.js";
import "./chunk-zw75zfcp.js";
import "./chunk-ksctmzwy.js";
import "./chunk-9fjy6xex.js";
import "./chunk-b4cswg8c.js";
import "./chunk-40xnyyhy.js";
import "./chunk-mrsbyrhq.js";
import "./chunk-30q5ynkm.js";
import "./chunk-w75z743m.js";
import "./chunk-qjj8swk2.js";
import "./chunk-tkq0r7ym.js";
import "./chunk-c5577t1e.js";
import "./chunk-eza32dw3.js";
import "./chunk-11mcqc6q.js";
import "./chunk-5gp3752w.js";
import "./chunk-vpt6j0hj.js";
import "./chunk-gbnf6vz0.js";
import "./chunk-bqzk1zdc.js";
import "./chunk-sm4f0pvs.js";
import "./chunk-1q5tty9h.js";
import "./chunk-nh5b9j9c.js";
import "./chunk-bzxfr890.js";
import "./chunk-71edvt4b.js";
import "./chunk-jrks771b.js";
import "./chunk-m97fmp5a.js";
import "./chunk-yz9ewb10.js";
import "./chunk-31k5d81d.js";
import "./chunk-s98sa0gn.js";
import "./chunk-meget4pa.js";
import "./chunk-b9h3hrm9.js";
import "./chunk-y91b9t7k.js";
import "./chunk-t8rh9pbw.js";
import "./chunk-wyqz2xfr.js";
import "./chunk-bzm4f2ge.js";
import "./chunk-7b08b87y.js";
import "./chunk-0697cd2s.js";
import "./chunk-1kx3xrw0.js";
import "./chunk-8gvda1c5.js";
import "./chunk-752tnqtv.js";
import "./chunk-6nznq4h0.js";
import "./chunk-075s54dh.js";
import "./chunk-pgmqkm8j.js";
import "./chunk-cj0gpwwk.js";
import "./chunk-qpqhpjnf.js";
import "./chunk-9tnnrncs.js";
import "./chunk-b3pk8jsy.js";
import "./chunk-xxq66zap.js";
import "./chunk-67zd0pq0.js";
import "./chunk-13k2svs7.js";
import "./chunk-kxcfgh7d.js";
import "./chunk-abnm2sa9.js";
import "./chunk-m1g84jsb.js";
import "./chunk-jea0jsvq.js";
import "./chunk-f7czbtwe.js";
import "./chunk-grn2cpxb.js";
import "./chunk-s2qv9t07.js";
import "./chunk-h3nnndyy.js";
import "./chunk-68asg28d.js";
import "./chunk-at3x5y7v.js";
import "./chunk-wfap5c8g.js";
import "./chunk-xdnjqx4p.js";
import "./chunk-z98654x2.js";
import "./chunk-8p3wyg4e.js";
import "./chunk-4jxzmesc.js";
import "./chunk-wevssd80.js";
import "./chunk-f1sf39vv.js";
import "./chunk-se2pd50z.js";
import "./chunk-rrerhb6z.js";
import { xee } from "./chunk-c5d41c9f.js";
import "./chunk-wq3c0qxx.js";
import "./chunk-ec7k1p7y.js";
import "./chunk-82z04ms9.js";
import "./chunk-j46hdw8h.js";
import { wo } from "./chunk-k3yy1efd.js";
import "./chunk-g162cr20.js";
import "./chunk-e1vsnv9j.js";
import "./chunk-qfwnyscs.js";
import "./chunk-sgq4jt66.js";
// import "./chunk-f7zydv5a.js"; // thy
import { e, r } from "./chunk-g3erx2ps.js";
// import "./chunk-19edp2ga.js"; // thy
import "./chunk-jejcqxww.js";
import "./chunk-zeh1n4wq.js";
import "./chunk-gmdrem8f.js";
import { v, K, d, j } from "./chunk-w71cs9yg.js";
import "./chunk-rpq8fknf.js";
import "./chunk-4amf009w.js";
import "./chunk-zbjsms1x.js";
import "./chunk-gt0sz7qx.js";
import "./chunk-8cb3kpz7.js";
import "./chunk-kvmgr8kf.js";
import "./chunk-gyc9r9jx.js";
import "./chunk-fpm5782j.js";
import "./chunk-g41jh2m0.js";
import "./chunk-mz0qwadn.js";
import "./chunk-eft7b88m.js";
import "./chunk-nzscmy2v.js";
import "./chunk-e4vwkrtj.js";
import "./chunk-6dth79yw.js";
import "./chunk-gj0499cq.js";
import "./chunk-szzsnrz5.js";
import "./chunk-pq20xdar.js";
import "./chunk-h9mewq95.js";
import "./chunk-a3vshzn8.js";
import "./chunk-3e1zwnk7.js";
import "./chunk-7bfx680p.js";
import "./chunk-4796xb9w.js";
import "./chunk-31e4aq9f.js";
import "./chunk-90gearh2.js";
import "./chunk-wtntbnrk.js";
import "./chunk-mh620fe5.js";
import "./chunk-h2yhfh1h.js";
import "./chunk-y0c7xssj.js";
import "./chunk-7nw1s99d.js";
import "./chunk-f6ht09n5.js";
import "./chunk-vckrj367.js";
import "./chunk-zc7jwbz1.js";
import "./chunk-50etkfry.js";
import "./chunk-c6eb44np.js";
import "./chunk-nc8ww32a.js";
import "./chunk-my1n9ey3.js";
import "./chunk-v10h0yg2.js";
import "./chunk-5m6nm8br.js";
import { p } from "./chunk-qyvz15br.js";
j();
j();
import { sep as ao } from "path";
function cr(Ir) {
  return Ir;
}
function lr(Vr) {
  return `No color named "${Vr}"`;
}
var uo = ARn + ARn;
function Q(Br) {
  let Ar = y(2),
    { value: Ge } = Br,
    Lo;
  if (Ar[0] !== Ge) ((Lo = e(n, { color: Ge, children: uo })), (Ar[0] = Ge), (Ar[1] = Lo));
  else Lo = Ar[1];
  return Lo;
}
function Ee(Fr) {
  let s = y(152),
    { initial: P, defaultBase: Ie, onDone: Ve, onCancel: ae } = Fr,
    { storageV5: de } = ye(),
    [, Ze] = yn(),
    { customThemes: He, reloadCustomThemes: Se, setPreviewOverrides: u } = E9(),
    eo = P !== void 0 && P.source !== "user",
    [xo, Ur] = d(P && !eo ? "colors" : "name"),
    [l, Mo] = d(P?.name ?? ""),
    [oo, Qr] = d(l.length),
    [f, qr] = d(eo ? "" : (P?.slug ?? "")),
    jo;
  if (s[0] !== Ie || s[1] !== P?.base)
    ((jo = () => P?.base ?? Ie), (s[0] = Ie), (s[1] = P?.base), (s[2] = jo));
  else jo = s[2];
  let [h] = d(jo),
    Bo;
  if (s[3] !== h) ((Bo = ZC(h)), (s[3] = h), (s[4] = Bo));
  else Bo = s[4];
  let C = Bo,
    Ao;
  if (s[5] !== P?.overrides) ((Ao = P?.overrides ?? {}), (s[5] = P?.overrides), (s[6] = Ao));
  else Ao = s[6];
  let [m, Jr] = d(Ao),
    Fo;
  if (s[7] !== C) ((Fo = Object.keys(C).sort()), (s[7] = C), (s[8] = Fo));
  else Fo = s[8];
  let Ae = Fo,
    [ue, Xr] = d(""),
    [a, ro] = d(null),
    [E, Uo] = d(""),
    [so, Qo] = d(0),
    qo;
  if (s[9] !== Ae || s[10] !== ue) {
    let Jo = ue.toLowerCase();
    qo = Jo ? Ae.filter((_r) => _r.toLowerCase().includes(Jo)) : Ae;
    ((s[9] = Ae), (s[10] = ue), (s[11] = qo));
  } else qo = s[11];
  let to = qo,
    Xo;
  if (s[12] !== He || s[13] !== l || s[14] !== f)
    ((Xo = f || fo(l, He)), (s[12] = He), (s[13] = l), (s[14] = f), (s[15] = Xo));
  else Xo = s[15];
  let z = Xo,
    _o;
  if (s[16] !== m) ((_o = Object.keys(m)), (s[16] = m), (s[17] = _o));
  else _o = s[17];
  let Te = _o.length,
    zo,
    Ko;
  if (s[18] !== u)
    ((zo = () => () => u(null)), (Ko = [u]), (s[18] = u), (s[19] = zo), (s[20] = Ko));
  else ((zo = s[19]), (Ko = s[20]));
  v(zo, Ko);
  let Yo;
  if (s[21] !== m || s[22] !== C)
    ((Yo = function L(Go) {
      return m[Go] ?? C[Go];
    }),
      (s[21] = m),
      (s[22] = C),
      (s[23] = Yo));
  else Yo = s[23];
  let L = Yo,
    Io;
  if (s[24] !== h || s[25] !== l || s[26] !== u || s[27] !== de)
    ((Io = function S(Vo, no) {
      (Jr(no),
        u(no),
        Z4e({ slug: Vo, name: l.trim(), base: h, overrides: no, source: "user" }, de).catch(
          (zr) => {
            t(`[theme] save ${Vo} failed: ${zr}`, { level: "warn" });
          },
        ));
    }),
      (s[24] = h),
      (s[25] = l),
      (s[26] = u),
      (s[27] = de),
      (s[28] = Io));
  else Io = s[28];
  let S = Io,
    Wo;
  if (s[29] !== L)
    ((Wo = function be(Zo) {
      let Ho = L(Zo);
      (Uo(Ho), Qo(Ho.length), ro(Zo));
    }),
      (s[29] = L),
      (s[30] = Wo));
  else Wo = s[30];
  let be = Wo,
    So;
  if (s[31] !== S || s[32] !== E || s[33] !== a || s[34] !== m || s[35] !== C || s[36] !== f)
    ((So = function $e() {
      if (a === null || !Z2(E)) {
        return;
      }
      (S(f, E === C[a] ? Il(m, a) : { ...m, [a]: E }), ro(null));
    }),
      (s[31] = S),
      (s[32] = E),
      (s[33] = a),
      (s[34] = m),
      (s[35] = C),
      (s[36] = f),
      (s[37] = So));
  else So = s[37];
  let $e = So,
    er;
  if (s[38] !== m || s[39] !== u)
    ((er = function ee() {
      (u(m), ro(null));
    }),
      (s[38] = m),
      (s[39] = u),
      (s[40] = er));
  else er = s[40];
  let ee = er,
    or;
  if (s[41] !== S || s[42] !== m || s[43] !== f)
    ((or = function De(rr) {
      if (!(rr in m)) {
        return;
      }
      S(f, Il(m, rr));
    }),
      (s[41] = S),
      (s[42] = m),
      (s[43] = f),
      (s[44] = or));
  else or = s[44];
  let De = or,
    sr;
  if (s[45] !== a || s[46] !== m || s[47] !== u)
    ((sr = function ke(io) {
      if ((Uo(io), a && Z2(io))) u({ ...m, [a]: io });
    }),
      (s[45] = a),
      (s[46] = m),
      (s[47] = u),
      (s[48] = sr));
  else sr = s[48];
  let ke = sr,
    tr;
  if (s[49] !== ee || s[50] !== a || s[51] !== ae)
    ((tr = () => {
      if (a !== null) ee();
      else ae();
    }),
      (s[49] = ee),
      (s[50] = a),
      (s[51] = ae),
      (s[52] = tr));
  else tr = s[52];
  const mo = xo === "name" || a !== null;
  let nr;
  if (s[53] !== mo) ((nr = { context: "Settings", isActive: mo }), (s[53] = mo), (s[54] = nr));
  else nr = s[54];
  if ((We("confirm:no", tr, nr), xo === "name")) {
    let Y;
    if (s[55] !== l) ((Y = l.trim()), (s[55] = l), (s[56] = Y));
    else Y = s[56];
    let Pe = Y;
    let fe = Pe.length > 0;
    const N = eo && P ? `Fork ${P.name} to your themes` : "New custom theme";
    let O;
    if (s[57] !== N)
      ((O = e(n, { bold: !0, color: "permission", children: N })), (s[57] = N), (s[58] = O));
    else O = s[58];
    let w;
    if (s[59] === p) ((w = e(n, { children: "Name: " })), (s[59] = w));
    else w = s[59];
    let x;
    if (
      s[60] !== h ||
      s[61] !== z ||
      s[62] !== m ||
      s[63] !== Se ||
      s[64] !== Ze ||
      s[65] !== de ||
      s[66] !== Pe ||
      s[67] !== fe
    )
      ((x = () => {
        if (!fe) {
          return;
        }
        (qr(z),
          Mo(Pe),
          Ur("colors"),
          Z4e({ slug: z, name: Pe, base: h, overrides: m, source: "user" }, de)
            .then(() => Se())
            .then(() => {
              Ze(jz(z));
            })
            .catch((Kr) => {
              t(`[theme] save ${z} failed: ${Kr}`, { level: "warn" });
            }));
      }),
        (s[60] = h),
        (s[61] = z),
        (s[62] = m),
        (s[63] = Se),
        (s[64] = Ze),
        (s[65] = de),
        (s[66] = Pe),
        (s[67] = fe),
        (s[68] = x));
    else x = s[68];
    let b;
    if (s[69] !== l || s[70] !== oo || s[71] !== ae || s[72] !== x)
      ((b = r(o, {
        children: [
          w,
          e(xn, {
            value: l,
            onChange: Mo,
            onSubmit: x,
            onExit: ae,
            placeholder: "my-theme",
            columns: 40,
            cursorOffset: oo,
            onChangeCursorOffset: Qr,
            disableCursorMovementForUpDownKeys: !0,
            disableEscapeDoublePress: !0,
            focus: !0,
            showCursor: !0,
          }),
        ],
      })),
        (s[69] = l),
        (s[70] = oo),
        (s[71] = ae),
        (s[72] = x),
        (s[73] = b));
    else b = s[73];
    let U;
    if (s[74] === p) ((U = mHe()), (s[74] = U));
    else U = s[74];
    let oe;
    if (s[75] !== h || s[76] !== z)
      ((oe = r(n, {
        dimColor: !0,
        children: ["based on ", h, " \xB7 saved to ", U, ao, z, ".json"],
      })),
        (s[75] = h),
        (s[76] = z),
        (s[77] = oe));
    else oe = s[77];
    let G;
    if (s[78] !== b || s[79] !== oe)
      ((G = r(o, { flexDirection: "column", children: [b, oe] })),
        (s[78] = b),
        (s[79] = oe),
        (s[80] = G));
    else G = s[80];
    let I;
    if (s[81] !== fe)
      ((I = fe && e(F, { chord: "enter", action: "continue" })), (s[81] = fe), (s[82] = I));
    else I = s[82];
    let Ne;
    if (s[83] === p) ((Ne = e(F, { chord: "escape", action: "cancel" })), (s[83] = Ne));
    else Ne = s[83];
    let V;
    if (s[84] !== I)
      ((V = e(n, { dimColor: !0, children: r(pe, { children: [I, Ne] }) })),
        (s[84] = I),
        (s[85] = V));
    else V = s[85];
    let ie;
    if (s[86] !== O || s[87] !== G || s[88] !== V)
      ((ie = e(wo, {
        color: "permission",
        children: r(o, { flexDirection: "column", gap: 1, children: [O, G, V] }),
      })),
        (s[86] = O),
        (s[87] = G),
        (s[88] = V),
        (s[89] = ie));
    else ie = s[89];
    return ie;
  }
  if (a !== null) {
    let Y;
    if (s[90] !== E) ((Y = Z2(E)), (s[90] = E), (s[91] = Y));
    else Y = s[91];
    let Fe = Y;
    let N;
    if (s[92] !== l)
      ((N = e(n, { bold: !0, color: "permission", children: l })), (s[92] = l), (s[93] = N));
    else N = s[93];
    const O = Fe ? E : C[a];
    let w;
    if (s[94] !== O) ((w = e(Q, { value: O })), (s[94] = O), (s[95] = w));
    else w = s[95];
    let x;
    if (s[96] === p) ((x = e(n, { children: " " })), (s[96] = x));
    else x = s[96];
    let b;
    if (s[97] !== a) ((b = e(n, { bold: !0, children: a })), (s[97] = a), (s[98] = b));
    else b = s[98];
    let U;
    if (s[99] !== w || s[100] !== b)
      ((U = r(o, { children: [w, x, b] })), (s[99] = w), (s[100] = b), (s[101] = U));
    else U = s[101];
    const oe = C[a];
    let G;
    if (s[102] !== oe)
      ((G = r(n, { dimColor: !0, children: ["preset: ", oe] })), (s[102] = oe), (s[103] = G));
    else G = s[103];
    let I;
    if (s[104] !== U || s[105] !== G)
      ((I = r(o, { flexDirection: "column", children: [U, G] })),
        (s[104] = U),
        (s[105] = G),
        (s[106] = I));
    else I = s[106];
    let Ne;
    if (s[107] === p) ((Ne = e(n, { children: "Value: " })), (s[107] = Ne));
    else Ne = s[107];
    let V;
    if (s[108] !== ee || s[109] !== $e || s[110] !== so || s[111] !== E || s[112] !== ke)
      ((V = r(o, {
        children: [
          Ne,
          e(xn, {
            value: E,
            onChange: ke,
            onSubmit: $e,
            onExit: ee,
            placeholder: "rgb(r,g,b) \xB7 #rrggbb \xB7 ansi:red",
            columns: 40,
            cursorOffset: so,
            onChangeCursorOffset: Qo,
            disableCursorMovementForUpDownKeys: !0,
            disableEscapeDoublePress: !0,
            focus: !0,
            showCursor: !0,
          }),
        ],
      })),
        (s[108] = ee),
        (s[109] = $e),
        (s[110] = so),
        (s[111] = E),
        (s[112] = ke),
        (s[113] = V));
    else V = s[113];
    let ie;
    if (s[114] !== Fe)
      ((ie = Fe
        ? r(pe, {
            children: [
              e(F, { chord: "enter", action: "save" }),
              e(F, { chord: "escape", action: "cancel" }),
            ],
          })
        : "Accepts rgb(r,g,b), #rrggbb, ansi256(n), or ansi:name"),
        (s[114] = Fe),
        (s[115] = ie));
    else ie = s[115];
    let Ue;
    if (s[116] !== ie) ((Ue = e(n, { dimColor: !0, children: ie })), (s[116] = ie), (s[117] = Ue));
    else Ue = s[117];
    let ir;
    if (s[118] !== N || s[119] !== I || s[120] !== V || s[121] !== Ue)
      ((ir = e(wo, {
        color: "permission",
        children: r(o, { flexDirection: "column", gap: 1, children: [N, I, V, Ue] }),
      })),
        (s[118] = N),
        (s[119] = I),
        (s[120] = V),
        (s[121] = Ue),
        (s[122] = ir));
    else ir = s[122];
    return ir;
  }
  const Y = `${l} \xB7 based on ${h}`;
  let N;
  if (s[123] !== De)
    ((N = {
      action: "reset",
      handler: (mr) => {
        if (mr) De(mr);
      },
    }),
      (s[123] = De),
      (s[124] = N));
  else N = s[124];
  let O;
  if (s[125] !== h || s[126] !== l || s[127] !== Ve || s[128] !== m || s[129] !== u || s[130] !== f)
    ((O = () => {
      (u(null), Ve({ slug: f, name: l, base: h, overrides: m, source: "user" }));
    }),
      (s[125] = h),
      (s[126] = l),
      (s[127] = Ve),
      (s[128] = m),
      (s[129] = u),
      (s[130] = f),
      (s[131] = O));
  else O = s[131];
  let w;
  if (s[132] !== Te || s[133] !== f)
    ((w = Te > 0 ? `${Te} ${R(Te, "color")} customized \xB7 ${f}.json` : `editing ${f}.json`),
      (s[132] = Te),
      (s[133] = f),
      (s[134] = w));
  else w = s[134];
  let x;
  if (s[135] !== m || s[136] !== L)
    ((x = (co, Yr) => {
      let Gr = m[co] !== void 0;
      return r(o, {
        children: [
          e(Q, { value: L(co) }),
          e(n, { children: " " }),
          e(n, { color: Yr ? "suggestion" : void 0, children: co }),
          e(aR, { when: Gr, children: "custom" }),
        ],
      });
    }),
      (s[135] = m),
      (s[136] = L),
      (s[137] = x));
  else x = s[137];
  let b;
  if (s[138] !== m || s[139] !== C || s[140] !== L)
    ((b = (Oe) =>
      r(o, {
        flexDirection: "column",
        children: [
          r(n, { children: ["current: ", e(Q, { value: L(Oe) }), " ", L(Oe)] }),
          m[Oe] !== void 0 &&
            r(n, { dimColor: !0, children: ["preset: ", e(Q, { value: C[Oe] }), " ", C[Oe]] }),
        ],
      })),
      (s[138] = m),
      (s[139] = C),
      (s[140] = L),
      (s[141] = b));
  else b = s[141];
  let U;
  if (
    s[142] !== to ||
    s[143] !== ue ||
    s[144] !== be ||
    s[145] !== Y ||
    s[146] !== N ||
    s[147] !== O ||
    s[148] !== w ||
    s[149] !== x ||
    s[150] !== b
  )
    ((U = e(jGe, {
      title: Y,
      placeholder: "Filter color tokens\u2026",
      items: to,
      getKey: cr,
      initialQuery: ue,
      onQueryChange: Xr,
      onSelect: be,
      onTab: N,
      onCancel: O,
      selectAction: "edit",
      cancelAction: "done",
      matchLabel: w,
      renderItem: x,
      renderPreview: b,
      emptyMessage: lr,
    })),
      (s[142] = to),
      (s[143] = ue),
      (s[144] = be),
      (s[145] = Y),
      (s[146] = N),
      (s[147] = O),
      (s[148] = w),
      (s[149] = x),
      (s[150] = b),
      (s[151] = U));
  else U = s[151];
  return U;
}
function fo(c, H) {
  let q = Fcn(c);
  if (!H.some((B) => B.slug === q)) return q;
  for (let B = 2; ; B++) {
    let J = `${q}-${B}`;
    if (!H.some((re) => re.slug === J)) return J;
  }
}
function vo(ls) {
  let M = y(25),
    { onDone: W } = ls,
    [po, he] = yn(),
    { customThemes: ho } = E9(),
    ar;
  if (M[0] === p) ((ar = { kind: "picker" }), (M[0] = ar));
  else ar = M[0];
  let [Qe, dr] = d(ar),
    ur;
  if (M[1] === p) ((ur = lo("themes")), (M[1] = ur));
  else ur = M[1];
  let fr = ur,
    go = w9(),
    pr;
  if (M[2] !== go) ((pr = gH(go)), (M[2] = go), (M[3] = pr));
  else pr = M[3];
  let qe = pr;
  if (Qe.kind === "editor") {
    let Z;
    if (M[4] !== W || M[5] !== he)
      ((Z = (hr) => {
        (he(jz(hr.slug)), W(`Using custom theme "${hr.name}"`));
      }),
        (M[4] = W),
        (M[5] = he),
        (M[6] = Z));
    else Z = M[6];
    let Re;
    if (M[7] === p) ((Re = () => dr({ kind: "picker" })), (M[7] = Re));
    else Re = M[7];
    let me;
    if (M[8] !== po || M[9] !== Qe.initial || M[10] !== Z)
      ((me = e(Ee, { initial: Qe.initial, defaultBase: po, onDone: Z, onCancel: Re })),
        (M[8] = po),
        (M[9] = Qe.initial),
        (M[10] = Z),
        (M[11] = me));
    else me = M[11];
    return me;
  }
  let Z;
  if (M[12] !== ho || M[13] !== W || M[14] !== he)
    ((Z = (Le) => {
      (he(Le),
        W(
          gH(Le)
            ? `Using custom theme "${ho.find((as) => jz(as.slug) === Le)?.name ?? Le}"`
            : `Theme set to ${Le}`,
        ));
    }),
      (M[12] = ho),
      (M[13] = W),
      (M[14] = he),
      (M[15] = Z));
  else Z = M[15];
  let Re;
  if (M[16] === p) ((Re = fr ? void 0 : (ds) => dr({ kind: "editor", initial: ds })), (M[16] = Re));
  else Re = M[16];
  let me;
  if (M[17] !== qe)
    ((me = fr
      ? `Custom themes are disabled in safe mode \u2014 ${kp()} to create or edit them${qe ? `. Your saved theme "${qe}" is a custom theme; selecting a preset here replaces it` : ""}`
      : ""),
      (M[17] = qe),
      (M[18] = me));
  else me = M[18];
  let Je;
  if (M[19] !== W)
    ((Je = () => {
      W("Theme picker dismissed", { display: "system" });
    }),
      (M[19] = W),
      (M[20] = Je));
  else Je = M[20];
  let gr;
  if (M[21] !== Z || M[22] !== me || M[23] !== Je)
    ((gr = e(wo, {
      color: "permission",
      children: e(xee, {
        onThemeSelect: Z,
        onCustomTheme: Re,
        helpText: me,
        onCancel: Je,
        skipExitHandling: !0,
      }),
    })),
      (M[21] = Z),
      (M[22] = me),
      (M[23] = Je),
      (M[24] = gr));
  else gr = M[24];
  return gr;
}
var ms = async (c, H) => e(vo, { onDone: c });
export { ms as call };
