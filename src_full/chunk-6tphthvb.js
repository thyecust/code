...
import { lut } from "./chunk-fdhdeqxf.js";
...
import { Ch, _x, ms } from "./chunk-x722nt0q.js";
import { eje, tje } from "./chunk-77152aqa.js";
import { fe } from "./chunk-gbnf6vz0.js";
import { Nf, iP, foe } from "./chunk-6nznq4h0.js";
import { tl, vEe, DJn, XJ } from "./chunk-qpwbvc04.js";
import { fte } from "./chunk-z3ewj87g.js";
...
function A(t, e) {
  let n = Nf().lastExecutedTabUrlByScope.get(t);
  if (n === void 0) return;
  return G(n, e) ? { from: n, to: e } : void 0;
}
function E(t, e) {
  Nf().lastExecutedTabUrlByScope.set(t, e);
}
var D = 200;
function O(t, e) {
  let n = Nf().resolvedHostByToolUseId;
  if (n.size >= D) n.clear();
  n.set(t, e);
}
function j(t, e) {
  let n = Nf().resolvedUrlByToolUseId;
  if (n.size >= D) n.clear();
  n.set(t, e);
}
function F() {
...
function RRr(t, e) {
  Nf().bridgeBinding = { context: t, socketClient: e };
}
var z = new RegExp(`^${foe}\\(([^)]+)\\)$`);
function x(t) {
  let e = new Set(),
    n = [],
    r = new Set(),
    o = new Map();
  return (
    L(t.alwaysDenyRules, r, o),
    L(t.alwaysAllowRules, e, o, n, r),
    { allowed: e, denied: r, allowedRaw: n, sourceOf: o }
  );
}
function q(t, e) {
  let n = iP.slice(0, -2),
  ...
  for (let [s, i] of Object.entries(t))
    for (let m of i ?? []) {
      ...
      if (d?.[1]) {
        let l = y(d[1]);
        if (o && N(o, l)) continue;
        if (!e.has(l)) (e.add(l), n.set(l, s), r?.push(d[1]));
      }
    }
}
function k(t) {
...
var V = new Set([
  ...
  W = new Set(["switch_browser", "select_browser"]),
  K = {
    tab_missing:
...
function X(t, e) {
  ...
  if (t === "browser_batch") {
    ...
    for (let s of e.actions) {
      if (!Ee(s) || typeof s.name !== "string" || s.name === "browser_batch")
        return { kind: "deny", code: "batch_malformed_item" };
      n.push({ name: s.name, input: Ee(s.input) ? s.input : {} });
    }
        ...
        let r = n.result?.content,
          o =
            ...
            typeof r[0].text === "string"
              ...
              : void 0;
        if (!o) return;
        return Y(o).availableTabs?.find((i) => i.tabId === t)?.url;
      })(),
      J,
...
async function te(t, e) {
  ...
  for (let o of t.content ?? [])
    if (o.type === "text") r.push({ type: "text", text: o.text });
      ...
      if (ee(o.mimeType))
        try {
          let { block: s } = await Qg({ data: String(o.data), mediaType: o.mimeType, limits: n });
          r.push(s);
        } catch {
...
function ne(t) {
  ...
  for (let e of t.actions) {
    if (!Ee(e) || typeof e.name !== "string") continue;
    let n = Ee(e.input) ? e.input : {};
    if (xDe.has(e.name) && vEe(e.name, n)) continue;
    if (
      e.name === "navigate" &&
      typeof n.url === "string" &&
...
function oe(t) {
  ...
  return t.actions.some((e) => {
    if (!Ee(e) || typeof e.name !== "string") return !0;
    return (
      e.name === "browser_batch" || (xDe.has(e.name) && !vEe(e.name, Ee(e.input) ? e.input : {}))
    );
  });
}
function re(t, e, n) {
...
async function ie(t, e, n, r) {
  ...
  if (s.aborted) throw new tt("Claude in Chrome tool call aborted");
  let i,
    m = new Promise((u, p) => {
      ((i = () => p(new tt("Claude in Chrome tool call aborted"))),
        s.addEventListener("abort", i, { once: !0 }));
    }),
    d;
  try {
    d = await Promise.race([lut(o.context, o.socketClient, t, e ?? {}, r), m]);
  } finally {
    if (i) s.removeEventListener("abort", i);
  }
  if (d.isError) {
    let u =
        (d.content ?? []).flatMap((b) => (b.type === "text" ? [b.text] : [])).join(`
`) || `${t} failed`,
      p = d._meta,
      w =
        p?.isBridgeTimeout === !0
          ...
          : p?.isFrontLoadBoundExceeded === !0
            ...
            : void 0;
    throw new C(u, "Claude in Chrome tool returned error", `chrome_${w ?? aut(t, u)}`);
  }
  let l = await te(d, n.options.mainLoopModel),
    a = fte(d._meta);
...
function rin(t) {
  ...
  return {
    checkPermissions: async (o, s) => {
      let i = s.toolUseId,
        m = xDe.has(t);
      if (m && vEe(t, o)) return { behavior: "allow", updatedInput: o };
      let d = (m && !vEe(t, o)) || (t === "browser_batch" && oe(o));
      if (
        t === "navigate" &&
        typeof o.url === "string" &&
      ...
      let l = t === "browser_batch" ? ne(o) : { input: o, toolName: t },
        a;
      ...
      let u = fe(s),
        p = _x(
          s.options?.tools?.find((c) => en(c, e)),
          u,
        ),
        w = p === "bypassPermissions" || (p === "plan" && u.isBypassPermissionsModeAvailable),
        b =
          u.chromeNavigationClassifierEnabled === !0 &&
          !w &&
          (u.chromeClassifierFloorEnabled === !0 || Ch(p)),
        h,
        T,
        I = () => {
          if (!i) return;
          if (T === void 0) Nf().resolvedUrlByToolUseId.delete(i);
          else j(i, T);
          if (h === void 0) DJn(i);
        };
      if (b) {
        ...
        if (c.kind === "tab") {
          ...
          if (((h = A(Q(), S.url)), !(d && u.mode === "plan"))) T = S.url;
        }
      }
      I();
      let v = !1;
      if (a) {
        let c = x(u),
          R = y(a.host),
          S = c.sourceOf.get(R) ?? c.sourceOf.get(P(R)) ?? "session";
        if (N(c.denied, R))
          return (
            g("chrome_permission_prompt", "domain_rule_denied"),
            {
              ...
              decisionReason: {
                ...
                rule: {
                  source: S,
                  ruleBehavior: "deny",
                  ruleValue: { toolName: foe, ruleContent: a.host },
                },
              },
            }
          );
        if (
          ((v = c.allowed.has(R)), v && h === void 0 && u.chromeClassifierFloorEnabled !== !0 && !d)
        ) {
          if (i) O(i, a);
          return {
            behavior: "allow",
        ...
        if (i && (v || u.chromeClassifierFloorEnabled === !0 || !q(u, e))) O(i, a);
      }
      if (d && u.mode === "plan")
        return { behavior: "passthrough", message: "Claude in Chrome requires permission." };
      return {
        behavior: "ask",
        ...
        suggestions: a
          ? v
            ...
            : [
                {
                  type: "addRules",
                  rules: [{ toolName: foe, ruleContent: a.host }],
                  behavior: "allow",
                  destination: "session",
                },
              ]
        ...
        decisionReason: h
          ...
          : {
              type: "other",
              reason: a ? `Claude in Chrome action on ${a.host}` : "Claude in Chrome action",
            },
        metadata: {
          command: {
            name: e,
            description: re(t, o, a?.host),
            chrome: a ? { ...a, domainAllowed: v, navigation: h } : void 0,
          },
        },
      };
    },
    call: async (o, s) => {
      let i = s.toolUseId,
        m = i ? Nf().resolvedHostByToolUseId.get(i)?.host : void 0,
        d = i ? Nf().resolvedUrlByToolUseId.get(i) : void 0;
      if (i) (Nf().resolvedHostByToolUseId.delete(i), Nf().resolvedUrlByToolUseId.delete(i));
      let l = fe(s),
        a = s.options?.tools?.find((v) => en(v, e)),
        u = _x(a, l) === "bypassPermissions",
        p = x(l),
        w = p.allowed,
        b = [...p.allowedRaw];
      if (m && !w.has(y(m))) (w.add(y(m)), b.push(m));
      let h = F(),
        T = u
          ...
          : m
            ? {
                ...
                allowedDomains: b,
                onPermissionRequest: se(w),
                sessionScope: h,
              }
            : b.length > 0
              ? { permissionMode: "follow_a_plan", allowedDomains: b, sessionScope: h }
              : { permissionMode: "ask", sessionScope: h },
        I = await ie(t, o, s, T);
      if (d !== void 0) E(Q(), d);
      if (m) _("chrome_permission_prompt");
      return I;
    },
  };
}
