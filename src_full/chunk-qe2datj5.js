...
// Version: 2.1.259
import { cte } from "./chunk-kzwypryg.js";
...
async function R(r, e) {
  ...
  do {
    ...
    try {
      i = await Ei(r.client).request(
        { method: "resources/directory/read", params: { uri: e, ...(o && { cursor: o }) } },
        cte,
        { timeout: Ol() },
      );
    } catch (n) {
      if (s === 0 || !(n instanceof ta && n.code === ei.InvalidParams)) throw n;
      return (
        Z(
          r.name,
  ...
  if (o) Z(r.name, `resources/directory/read ${e}: stopped at ${a} pages with more pending`);
  return t;
}
