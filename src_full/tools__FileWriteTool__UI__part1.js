...
import { Cht, dGt } from "./chunk-qpwbvc04.js";
...
var O = 10,
  ...
  M = "</tool_use_error>";
function v(t) {
  return t.startsWith(L) && t.endsWith(M) ? t.slice(L.length, -M.length) : t;
}
function T(t) {
  return t.length > E ? `${le(t, E)}\u2026` : t;
}
function h(K) {
  let u = y(12),
    { lines: m, verbose: _, maxLines: b, color: g } = K,
    C;
  if (u[0] !== m || u[1] !== b || u[2] !== _)
    ((C = _ ? m : m.slice(0, b).map(T)), (u[0] = m), (u[1] = b), (u[2] = _), (u[3] = C));
  else C = u[3];
  let R = C,
    f = m.length - R.length,
    p;
...
`),
  ...
  let d;
  if (u[7] !== f)
    ((d = f > 0 ? r(n, { dimColor: !0, children: ["\u2026 +", f, " more lines"] }) : null),
      (u[7] = f),
      (u[8] = d));
  else d = u[8];
  let N;
  if (u[9] !== p || u[10] !== d)
    ((N = r(B, { children: [p, d] })), (u[9] = p), (u[10] = d), (u[11] = N));
  else N = u[11];
  return N;
}
function U(t) {
  if (t?.outcome !== "ok" || typeof t.content !== "string") return !1;
  let i = Cht(t.content);
  return i.length > O || i.some((s) => s.length > E);
}
function H(t, i, { verbose: s }) {
  ...
  return e(xe, {
    children: r(o, {
      ...
      children: [
        e(n, { children: dGt(t) }),
        e(h, { lines: Cht(t.content ?? ""), verbose: s, maxLines: O }),
      ],
    }),
  });
}
function V(t, { verbose: i }) {
  if (typeof t !== "string") return e(af, { result: t, verbose: i });
  let s = v(t),
    l = s.indexOf(`
`),
    c = En(l === -1 ? s : s.slice(0, l)),
    a = l === -1 ? [] : Cht(s.slice(l + 1));
  return e(xe, {
    children: r(o, {
      flexDirection: "column",
      children: [
        e(n, { color: "error", children: c }),
        e(h, { lines: a, verbose: i, maxLines: Yxe - 1, color: "error" }),
      ],
    }),
  });
}
export { U as isResultTruncated, H as renderToolResultMessage, V as renderToolUseErrorMessage };
