...
import { Cve, sKt, oG } from "./chunk-w75z743m.js";
...
import { sE, bp } from "./chunk-x722nt0q.js";
...
import { dv } from "./chunk-n8ev5fmn.js";
...
var g = "</cross-session-message>",
  R = "peer";
function F(s) {
  let a = oG.find((I) => s.startsWith(I)),
    h = (a ? s.slice(a.length) : s).match(/^<cross-session-message\b([^>]*)>/)?.[1] ?? "",
    k = h.match(/\bfrom="([^"]+)"/)?.[1],
    P = h.match(/\bfrom-name="([^"]+)"/)?.[1];
  return Q(P, k);
}
function Q(s, a) {
  return (s && sE(s)) || (a && sE(U(a))) || R;
}
function S(se) {
  let i = y(23),
    {
  ...
  if (i[0] !== v || i[1] !== u) {
    let d = u;
    let q = oG.find((ce) => d.startsWith(ce));
    if (q) d = d.slice(q.length);
    let t;
    if (i[4] !== u) ((t = F(u)), (i[4] = u), (i[5] = t));
    else t = i[5];
    f = t;
    let z = d.lastIndexOf(g) + g.length;
    if (z > g.length - 1) {
      let H = d.slice(z);
      if (Cve.includes(H) || (v === !0 && sKt.includes(H))) d = d.slice(0, z);
    }
    let x;
    if (i[6] === p) ((x = /\n<\/cross-session-message>$/), (i[6] = x));
    else x = i[6];
    O = d.replace(/^<cross-session-message[^>]*>\n/, "").replace(x, "");
    ((i[0] = v), (i[1] = u), (i[2] = f), (i[3] = O));
  } else ((f = i[2]), (O = i[3]));
  let m = O;
  if (ae) {
    let t;
    if (i[7] !== m) ((t = e(dv, { text: m })), (i[7] = m), (i[8] = t));
    else t = i[8];
    return t;
  }
  if (!ie) {
    let t;
    if (i[9] !== w || i[10] !== f || i[11] !== m)
      ((t = e(aj, { displayName: f, addMargin: w, fallbackLabel: R, body: m })),
        (i[9] = w),
        (i[10] = f),
        (i[11] = m),
        (i[12] = t));
  ...
  const t = w ? 1 : 0;
  let x;
  if (i[13] === p) ((x = tH(void 0)), (i[13] = x));
  else x = i[13];
  let J;
  if (i[14] === p) ((J = e(n, { "aria-hidden": !0, children: N.pointer })), (i[14] = J));
  else J = i[14];
  let B;
  if (i[15] !== f) ((B = r(n, { color: x, children: ["@ ", f, J] })), (i[15] = f), (i[16] = B));
  else B = i[16];
  let D;
  if (i[17] !== m)
    ((D = e(o, { paddingLeft: 2, children: e(rs, { stripPromptTags: !1, children: m }) })),
      (i[17] = m),
      (i[18] = D));
  else D = i[18];
  let K;
  if (i[19] !== t || i[20] !== B || i[21] !== D)
    ((K = r(o, { flexDirection: "column", marginTop: t, width: "100%", children: [B, D] })),
      (i[19] = t),
      (i[20] = B),
      (i[21] = D),
      (i[22] = K));
...
function U(s) {
  let { scheme: a, target: l } = bp(s);
  switch (a) {
    case "uds":
      return l.slice(Math.max(l.lastIndexOf("/"), l.lastIndexOf("\\")) + 1).replace(/\.sock$/, "");
    case "bridge":
      return "(untitled)";
    case "did":
    case "other":
...
export { S as UserCrossSessionMessage, F as crossSessionDisplayName, Q as peerSenderDisplayName };
