...
import { xc, lrr } from "./chunk-hnqd864x.js";
...
import { jte, bFt } from "./chunk-vqmm06g2.js";
...
function m(o) {
  return {
    ...
    loadedFrom: o.source === "built-in" ? "bundled" : o.source === "plugin" ? "plugin" : "skills",
    ...(o.source === "plugin" && {
      pluginInfo: {
        pluginManifest: o.pluginManifest,
        repository: o.plugin,
        ...(o.serverPluginId !== void 0 && { serverPluginId: o.serverPluginId }),
      },
    }),
    kind: "workflow",
...
Phases:
` +
            o.phases.map((r) => `- ${r.title}${r.detail ? `: ${r.detail}` : ""}`).join(`
`)
          : "",
        e = n.trim(),
        i = b(o.name),
        a = e ? `{ name: ${i}, args: ${b(e)} }` : `{ name: ${i} }`,
        l = Ute(t?.options?.tools)
          ? `

If the user asks you to modify this workflow or write a new script, load the \`${AA}\` skill first.`
          : "";
      return [
        {
          type: "text",
...
async function d(o, n) {
  if (!xc()) return [];
  return (await jte(o, n)).map(m);
}
export {
  m as createWorkflowCommand,
  d as getWorkflowCommands,
  bFt as invalidateWorkflowCache,
  g as warmWorkflows,
};
