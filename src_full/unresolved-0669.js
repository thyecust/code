...
import { hX, X1t, dV, PR, uO, uA, dA } from "./chunk-qpwbvc04.js";
...
var M = async (C, m) => {
  let t = ne(),
    d = X1t(m.getMcp().commands),
    s = (o) => hX(o, d),
    n = s(await dA(t, m.storageV5)),
    i = new Set(n.map((o) => o.name));
  (dV(), uA(), uO());
  let e = s(await dA(t, m.storageV5)),
    c = new Set(e.map((o) => o.name));
  PR.emit();
  let l = J(e, (o) => !i.has(o.name)),
    r = J(n, (o) => !c.has(o.name)),
    a = [];
  if (l > 0) a.push(`${l} added`);
  if (r > 0) a.push(`${r} removed`);
  let p = a.length > 0 ? a.join(", ") : "no changes",
    f = Dr() ? " (custom skills are disabled in safe mode)" : "";
  return {
    type: "text",
    value: `Reloaded skills: ${e.length} ${R(e.length, "skill")} available (${p})${f}`,
  };
};
export { M as call };
