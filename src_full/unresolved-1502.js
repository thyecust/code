...
import { d9 } from "./chunk-17cqagxf.js";
...
import { WVe, sPe, aPe, zVe } from "./chunk-92xc0hye.js";
import "./chunk-1ptznxfw.js";
import "./chunk-hq82kj5p.js";
import "./chunk-njve5wxt.js";
...
var M = async (t, o, r) => {
  let s = r?.trim() || "";
  if (!s) return e(d9, { onClose: t, context: o, defaultTab: "Config" });
  let n = s.toLowerCase();
  if (XI.includes(n) || IZ.includes(n)) {
    t(
...
${zVe(o)}`,
  ...
  let i = WVe(s);
  if (!i) {
    t(`Expected key=value, got "${Jn(s)}". Run /config to open settings.`, { display: "system" });
    return;
  }
...
function c(X) {
  let _ = y(6),
    { pairs: a, context: m, onDone: l } = X,
    [, p] = yn(),
    d = k(!1),
    u,
    C;
  if (_[0] !== m || _[1] !== l || _[2] !== a || _[3] !== p)
    ((u = () => {
      if (d.current) {
        return;
      }
      ((d.current = !0),
        sPe(a, m, { setTheme: p }).then((E) =>
          l(
            E.map((w) => aPe(w, Eo(m.session))).join(`
`),
            { display: "system" },
          ),
        ));
