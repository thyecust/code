...
import { _p } from "./chunk-x722nt0q.js";
...
import { n1n, FZt, CWe, cst } from "./chunk-z1871zqn.js";
...
function z(L) {
  ...
  return {
    label: `Version ${U}`,
    description: `${q.length} ${q.length === 1 ? "item" : "items"}`,
    value: U,
  };
}
var O = "__show_all__";
...
function h(s, t, i) {
  (t({ type: "append", messages: [Mt(s, "notice")] }), i(void 0, { display: "skip" }));
}
var ee = async (s, t) => {
  try {
    await Ut(FZt(t.storageV5), 500, "Timeout");
  } catch {}
  let i = await CWe(t.storageV5),
    g = cst(i)
      .slice()
      .sort(([a], [S]) => (_p(a, S) ? -1 : 1));
  if (g.length === 0) return (h(`See the full changelog at: ${n1n}`, t.applyMessageOp, s), null);
  return e(D, { notes: g, applyMessageOp: t.applyMessageOp, onDone: s });
};
function D(L) {
  let f = y(19),
  ...
  let M;
  if (f[0] !== X) ((M = { label: "Show all", description: X, value: O }), (f[0] = X), (f[1] = M));
  else M = f[1];
  let T;
  if (f[2] !== l || f[3] !== M) ((T = [M, ...l.map(z)]), (f[2] = l), (f[3] = M), (f[4] = T));
  else T = f[4];
  let _ = T,
    V = k(!1),
    B;
  ...
  if (f[9] !== c)
    ((v = function m() {
      ((V.current = !0), c(void 0, { display: "skip" }));
    }),
      (f[9] = c),
      (f[10] = v));
  else v = f[10];
  ...
  if (f[11] === p)
    ((E = e(o, {
      ...
      marginBottom: 1,
      children: e(n, { dimColor: !0, children: "Select a version to view its notes." }),
    })),
      (f[11] = E));
  else E = f[11];
  let A;
  if (f[12] !== m || f[13] !== C || f[14] !== _)
    ((A = e(Re, { options: _, visibleOptionCount: 10, onChange: C, onCancel: m })),
      (f[12] = m),
      (f[13] = C),
      (f[14] = _),
      (f[15] = A));
  ...
  let P;
  if (f[16] !== m || f[17] !== A)
    ((P = r(me, { title: "Release notes", onCancel: m, children: [E, A] })),
      (f[16] = m),
      (f[17] = A),
      (f[18] = P));
  else P = f[18];
...
export { D as ReleaseNotesPicker, ee as call, x as formatAll, w as formatVersion };
