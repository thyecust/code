...
// Version: 2.1.259
import { Zhe, eye } from "./chunk-6rkpsn9e.js";
...
import { HRn } from "./chunk-rahwxqh8.js";
...
import { F3 } from "./chunk-35w62chd.js";
...
import { V2, BOe, lbt } from "./chunk-qpwbvc04.js";
...
var ot = new RegExp(`^<${Zhe}\\s+source="([^"]*)"([^>]*)>\\n?`),
  f = `</${Zhe}>`,
  k = `
${f}`,
  st = /\buser="([^"]+)"/,
...
function V(c) {
  ...
  if (s.startsWith(V2)) {
    let t = s.indexOf(`
`);
    if (t !== -1 && s.startsWith(eye, t + 1)) ((i = s.slice(0, t)), (s = s.slice(t + 1)));
  }
  let m = ot.exec(s);
  if (!m) return null;
  let [w, x = "", R] = m;
  return { lead: i, rest: s, open: w, source: x, user: st.exec(R ?? "")?.[1] };
}
var Y = 60;
function it($t) {
  let u = y(34),
    { addMargin: W, param: dt, followsSpeakerLabel: X } = $t,
    { text: T } = dt,
    b = X === void 0 ? !1 : X,
    O,
    U,
  ...
  if (u[0] !== W || u[1] !== b || u[2] !== T) {
    ...
    bb0: {
      ...
      if (!z) {
        I = b
          ? e(dv, { text: T.trim() })
          : e(o, { marginTop: W ? 1 : 0, children: e(n, { children: T.trim() }) });
        break bb0;
      }
      let { open: xt, source: L, user: tt } = z;
      let { lead: g, rest: P } = z;
      let H = P.lastIndexOf(f) + f.length;
      if (H > f.length - 1) {
        let Rt = P.slice(H);
        if (J.includes(Rt)) P = P.slice(0, H);
      }
      let et = F3(L);
      if (g === `${V2}${et} while you were working:` || g === `${V2}${et}:`) g = "";
      let j = P.slice(xt.length);
      let nt = j.trimEnd();
      if (nt.endsWith(k)) j = nt.slice(0, -k.length);
      if (b) {
        const a = `${
          g
            ? `${g}
`
        ...
        let _;
        if (u[16] !== l) ((_ = e(dv, { text: l })), (u[16] = l), (u[17] = _));
        else _ = u[17];
        I = _;
        break bb0;
      }
      let ht = `${g ? `${g} ` : ""}${j}`.trim().replace(/\s+/g, " ");
      K = rt(ht, Y);
      B = o;
      F = W ? 1 : 0;
      U = n;
      if (u[18] === p)
        ((E = e(n, { "aria-label": "inbound:", color: "suggestion", children: HRn })), (u[18] = E));
      else E = u[18];
      D = " ";
      O = n;
      G = !0;
  ...
  if (u[30] !== B || u[31] !== l || u[32] !== F)
    ((_ = e(B, { marginTop: F, children: l })), (u[30] = B), (u[31] = l), (u[32] = F), (u[33] = _));
  else _ = u[33];
  return _;
}
export { it as UserChannelMessage, Q as displayServerName, V as parseChannelOpen };
